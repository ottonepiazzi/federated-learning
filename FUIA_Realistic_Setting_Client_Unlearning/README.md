...
are not code copies but **configuration files** in `experiments/` that override a few
parameters. This makes the differences between experiments explicit while keeping one
...
```
.
├── src/                  # single attack implementation (engine)
...
│   ├── fuia.py           # the 3 FUIA steps (separation, acquisition, inversion)
│   ├── attack.py         # attack orchestration + result saving
│   ├── metrics.py        # MSE, PSNR
│   └── main.py           # pipeline entry point
├── experiments/          # one config file per experiment (overrides only)
...
Alternatively, to run the pipeline with the default parameters (which match the
`baseline_difficult` experiment):
```bash
cd src
python main.py
```
...
and a target client holding 8 homogeneous images of a single class. The PSNR values are
those reported in the thesis (Table 4.6.1).
...
To add a new experiment, create a file `experiments/<name>.py` that redefines only the
parameters you want to change (e.g. `NUM_CLASSES`, `NUM_ROUNDS`, `FL_LR`, `TARGET_SIZE`,
`HOMOGENEOUS_TARGET`, `WEIGHTED_AGGREGATION`); all others keep the defaults from
`src/config.py`.
