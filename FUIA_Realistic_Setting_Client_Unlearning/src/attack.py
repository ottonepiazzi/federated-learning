...
from config import (NUM_ROUNDS, INV_RESTARTS, INV_ITERATIONS, TARGET_CLIENT,
                    DEVICE, INV_DEVICE)
...
def run_fuia_attack(original_model, stored_updates, client_data, private_data,
    ...
    for i in client_data[target]:
        img = private_data[i][0]
        m, p = compute_metrics(img, recon_img)
        if p > psnr:
            best_idx, original_img, mse, psnr = i, img, m, p
    print(f"  Nearest of {len(client_data[target])} client images: idx {best_idx}")
    ...
    axes[0].set_title(f"Nearest original (label={target_label})", fontsize=14)
    axes[0].axis("off")
